...
public class PageRank extends Configured implements Tool {
	...
	public static class Map extends MapReduceBase implements Mapper<LongWritable, Text, Text, Text> {
		...
		private final Text thisLink = new Text();
		...
		public void map(LongWritable key, Text value, OutputCollector<Text, Text> output, Reporter reporter)
			...
			if (parts.length >= 2) {
				if (parts[1].length() < 10) { // A rank b c...
					...
					outputValue.set("info " + degree + " "
							+ builder);
					output.collect(thisLink, outputValue);

					outputValue.set(thisLink + " "
							+ rank + " "
							+ degree);
					if (parts.length >= 3) {
						for (int i = 2; i < parts.length; i++) {
		...
		public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter)
			...
			while (values.hasNext()) {
				...
				if (!parts[0].equals("info")) {//Not info message
					sum += Double.parseDouble(parts[1]) / Double.parseDouble(parts[2]);
				} else {// info message
					// numNodes++;
