...
public class GraphPartitioning {
            ...
            extends Mapper<LongWritable, Text, IntWritable, Text>    //<K_in, V_in, K_out, V_out>
    {
        ...
        public void map(LongWritable key, Text record/*edge "[src]<tab>[dst]"*/, Context context) throws IOException {
            ...
            double graphDensity = Double.parseDouble(line[2]);
            ...
            extends Reducer<IntWritable, Text, Text, Text>            //<K_in, V_in, K_out, V_out>
    {
        public void reduce(IntWritable key, Iterable<Text> edges, Context context) throws IOException {
            ...
            while (iter.hasNext()) {
                ...
                int src = Integer.parseInt(edge[0]);        //first node of this edge
                int dst = Integer.parseInt(edge[1]);        //second node of this edge
                ...
                if (!iter.hasNext()) graphDensity = (int) Math.round(Double.parseDouble(edge[2]));
            }

            Text output_edge = new Text();
            while (!queue.isEmpty()) {
                String[] edge_ = queue.poll().split("\\t");
                int src = Integer.parseInt(edge_[0]);
                int dst = Integer.parseInt(edge_[1]);
